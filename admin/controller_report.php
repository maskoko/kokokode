...
<?php
  ...
  if (isset($_GET['action']) and $_GET['action'] == "createReportPTK"):
	...
	header('Content-Type: application/vnd.ms-excel');
	header('Content-Disposition: attachment;filename="ptk.xls"');
	header('Cache-Control: max-age=0');
...
<?php
  ...
  if (isset($_GET['action']) and $_GET['action'] == "createReportTNA_PTK"):
	...
	$objPHPExcel->getProperties()->setCreator($core->company)
                                    ...
                                    ->setKeywords("PPPPTK BMTI Data Hasil TNA Online PTK xls")
                                    ->setCategory("Data Master");
  ...
  $worksheet->setCellValueByColumnAndRow(5, 4, 'Nilai %');
	...
	// Rename worksheet
	$worksheet->setTitle('TNA Online PTK');
	...
	// Redirect output to a clients web browser (Excel5)
	header('Content-Type: application/vnd.ms-excel');
	header('Content-Disposition: attachment;filename="hasil_tna_ptk.xls"');
	header('Cache-Control: max-age=0');
...
<?php
  ...
  if (isset($_GET['action']) and $_GET['action'] == "createReportDiklat_CalonPeserta"):
	...
	// Redirect output to a clients web browser (Excel5)
	header('Content-Type: application/vnd.ms-excel');
	header('Content-Disposition: attachment;filename="calonpeserta_diklat.xls"');
	header('Cache-Control: max-age=0');
...
<?php
  ...
  if (isset($_GET['action']) and $_GET['action'] == "createReportPTK_DiklatMinat"):
	...
	$objPHPExcel->getProperties()->setCreator($core->company)
                                    ->setLastModifiedBy($core->company)
                                    ->setTitle("Laporan Data Minat Diklat PTK")
                                    ->setSubject("Data Minat Diklat PTK")
                                    ->setDescription("Laporan Data Minat Diklat PTK.")
	...
	$worksheet->setCellValueByColumnAndRow(0, 1, 'Data Minat Diklat PTK');
	$worksheet->setCellValueByColumnAndRow(0, 2, 'Propinsi');
	$worksheet->setCellValueByColumnAndRow(0, 3, '');
	...
	// Rename worksheet
	$worksheet->setTitle('Minat Diklat PTK');
