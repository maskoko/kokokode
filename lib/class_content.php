  ...
  /**
        ...
        public function getKategori()
        {
              ...
              if (isset($_GET['sortfield'])) {
                  ...
                  if (in_array($sortfield, array("kode", "nama_kategori"))) {
                      $sort = ($sorttype == 'DESC') ? " DESC " : " ASC ";
        ...
        /**
         * Content::processAddKategori()
         * 
         * @return
         */
        public function processAddKategori()
        {
            ...
            if (empty($_POST['nama_kategori']))
                Filter::$msgs['nama_kategori'] = 'Silahkan masukkan nama Kategori';
        ...
        /**
         * Content::processUpdateKategori()
         * 
         * @return
         */
        public function processUpdateKategori()
        {
            ...
            if (empty(Filter::$msgs)) {
                $data = array('nama_kategori' => sanitize($_POST['nama_kategori']),
                              'last_update' => 'NOW()',
                              'userid' => intval($_POST['userid']));
                  
                self::$db->update("kategori", $data, "kode='" . Filter::$post['kode'] . "'");
                $message = lang('DATA_UPDATED');
        ...
        public function getMateri()
        {
              ...
              if (isset($_GET['sortfield'])) {
                  ...
                  if (in_array($sortfield, array("id", "nama_kategori"))) {
                      $sort = ($sorttype == 'DESC') ? " DESC " : " ASC ";
              ...
              $sql = "SELECT *"
                    . "\n FROM materi m,kategori k WHERE k.kode = m.kategoriid" 
                    . "\n ORDER BY " . $sqlorder . $pager->limit;
        ...
        /**
         * Content::getMateriList()
         * 
         * @return
         */
        public function getMateriList()
        {   
            $sql = "SELECT *"
                  . "\n FROM materi m,kategori k WHERE k.kode = m.kategoriid" 
                  . "\n ORDER BY nama_kategori";
        ...
        /**
         * Content::processAddMateri()
         * 
         * @return
         */
        public function processAddMateri()
        {
   
            if (empty($_POST['kategoriid']))
                Filter::$msgs['kategoriid'] = 'Silahkan pilih kategori';
   
            if (empty($_POST['deskripsi_materi']))
                Filter::$msgs['deskripsi_materi'] = 'Silahkan masukkan deskripsi Materi';
                      
            if (empty(Filter::$msgs)) {
                $data = array('kategoriid' => sanitize($_POST['kategoriid']),
                              'nama_materi' => sanitize($_POST['nama_materi']),
                              'deskripsi_materi' => sanitize($_POST['deskripsi_materi']),
                              'jam' => sanitize($_POST['jam']),
                              'last_update' => 'NOW()',
                              'created' => 'NOW()',
                              'userid' => intval($_POST['userid']));
                  
                self::$db->insert("materi", $data);
                $message = lang('DATA_ADDED');
        ...
        /**
         * Content::processUpdateMateri()
         * 
         * @return
         */
        public function processUpdateMateri()
        {
            ...
            if (empty(Filter::$msgs)) {
                $data = array('nama_materi' => sanitize($_POST['nama_materi']),
                'deskripsi_materi' => sanitize($_POST['deskripsi_materi']),
                'jam' => sanitize($_POST['jam']),
                              'last_update' => 'NOW()',
                              'userid' => intval($_POST['userid']));
                  
                self::$db->update("materi", $data, "id='" . Filter::$post['id'] . "'");
                $message = lang('DATA_UPDATED');
     ...
     /**
       ...
       * @return
       */
      public function getPTK_MMDS_jenissekolah($ptkid)
      {
          $sql = "SELECT mmds.*,"
                . "\n ptsk.jenisid"
                . "\n FROM ptk_mmds as mmds"
                . "\n LEFT JOIN (SELECT sek.jenisid, ptk.`id` FROM sekolah AS sek LEFT JOIN ptk ON ptk.`sekolahid` = sek.`id`) ptsk ON mmds.ptkid = ptsk.id"
                . "\n WHERE mmds.ptkid = '" . $ptkid . "' LIMIT 1";
      ...
      public function getPTK_MMDS($ptkid)
      {
          $sql = "SELECT mmds.*,"
                ...
                . "\n LEFT JOIN psk as psk ON mmds.kkid = psk.id"
                . "\n LEFT JOIN (SELECT sek.jenisid, ptk.`id` FROM sekolah AS sek LEFT JOIN ptk ON ptk.`sekolahid` = sek.`id`) ptsk ON mmds.ptkid = ptsk.id"
      ...
      /**
       * Content::getMMDMatPelOption()
       * 
       * @return
       */
      public function getMMDMatPelOption()
      {          
      ...
      /**
       * Users::checkPTK_NoPesUKG()
       * 
       * @param mixed $nip, $ptkid
       * @return
       */
      private function checkPTK_NOPESUKG($nopesukg, $ptkid)
      {
          $nopesukg = sanitize($nopesukg);
          if (strlen(self::$db->escape($nopesukg)) < 12)
              return 1; // -- digit kurang --

          // -- check apa numerik semua ? --

          if ($ptkid > 0)
            $sql = self::$db->query("SELECT id FROM ptk WHERE nopesukg = '" . $nopesukg . "' AND id <> " . $ptkid ." LIMIT 1");
          else
            $sql = self::$db->query("SELECT id FROM ptk WHERE nopesukg = '" . $nopesukg . "' LIMIT 1");
...
//webcam
			$camfoto = $_POST['camfoto'];

			if($camfoto){
				$nip = sanitize($_POST['nip']);
				$data = str_replace("data:image/jpeg;base64,","",$camfoto);
				$dir = date("dmY");  
				$targetdir = "../foto/foto_ptk/".$dir."/";   

				//cek apakah folder sudah pernah dibuat sebelumnya
                  if(!@opendir($targetdir) ){ 
                    //jika belum, buat dengan fungsi php mkdir      
                    @mkdir($targetdir);      
                  }
				
                //disini adalah fungsi untuk membuat nama gambar dengan kreasi sendiri
                $fnames = 'PTK_'.$nip.'.jpg';  
				$file = file_put_contents($targetdir.$fnames,base64_decode($data)); 
                  
                //kemudian jadikan sql data tadi menjadi berisi
                $filecomment=",'$fnames','$dir'"; 
                $foto = sanitize($fnames);
                $dir = sanitize($dir);
			}
            ...
            if($_FILES['foto']){
            //dapatkan ekstensi gambar yang diupload
            $file  = $_FILES['foto'];
            $ext_file = pathinfo($file['name'], PATHINFO_EXTENSION);  
            //pecah nama file dan ekstensi dari nama file menjadi dua variable, $fileName dan $ext
            list($fileName, $ext) = explode(".", $file['name']); 
            $filecomment=",'',''"; //siapkan sebuah variabel untuk data sql
            $nip = sanitize($_POST['nip']);
            $fname = $fileName.'.'.$ext_file;

            if($fname){

                  //dapatkan tanggal hari ini tanpa tanda. mis: 12122012 (12 desember 2012)
                  $dir = date("dmY");  
                  //siapkan lokasi folder tempat file gambar disimpan
                  //tambahkan juga folder gambar anda secara manual
                  $targetdir = "../foto/foto_ptk/".$dir."/";   
                  ...
                  //disini adalah fungsi untuk membuat nama gambar dengan kreasi sendiri
                  $fnames = 'PTK_'.$nip.'.'.$ext_file;  
                  
                  //upload/copy gambar ke lokasi folder yg sudah dibuat sebelumnya
                  $upload = @move_uploaded_file($file['tmp_name'], $targetdir.$fnames); 
                  //kemudian jadikan sql data tadi menjadi berisi
                  $filecomment=",'$fnames','$dir'"; 
                  $foto = sanitize($fnames);
                  $dir = sanitize($dir);
          ...
          if (empty($_POST['nopesukg']))
              Filter::$msgs['nopesukg'] = 'Silahkan masukkan NoPesUKG';

            if ($value = $this->checkPTK_NOPESUKG($_POST['nopesukg'], Filter::$id)) {
                if ($value == 1)
                    Filter::$msgs['nopesukg'] = 'Jumlah digit NoPesUKG kurang dari 12 karakter!';
                if ($value == 2)
                    Filter::$msgs['nopesukg'] = 'NoPesUKG tidak valid!';
                if ($value == 3)
                    Filter::$msgs['nopesukg'] = 'NoPesUKG sudah terdaftar!';
            }
          ...
          if (empty(Filter::$msgs)) {
                ...
                $data = array('nuptk' => sanitize($_POST['nuptk']),
                              ...
                              'dir_foto' => $dir,
                              'nip' => sanitize($_POST['nip']),
                              'nopesukg' => sanitize($_POST['nopesukg']),
	...
	//webcam
          ...
          if (empty($_POST['nopesukg']))
            ...
            if ($value = $this->checkPTK_NOPESUKG($_POST['nopesukg'], 0)) {
                if ($value == 1)
                    Filter::$msgs['nopesukg'] = 'Jumlah digit NoPesUKG kurang dari 12 karakter!';
                if ($value == 2)
                    Filter::$msgs['nopesukg'] = 'NoPesUKG tidak valid!';
      ...
      {	  
            ...
            $sql = "SELECT dj.id, dj.diklatid, dj.kelas, dj.tahun, DATE_FORMAT(dj.tgl_mulai, '%d/%m/%Y') AS tgl_mulai,"
     ...
     * @return
     */
    private function kelasExists($kelas)
    {
        $kelas = sanitize($kelas);

        $sql = self::$db->query("SELECT kelas FROM diklat_jadwal WHERE kelas = '" . $kelas . "' LIMIT 1");
	  ...
	  public function processDiklat_Jadwal()
	  {
                ...
                if (empty($_POST['kelas']))
                        Filter::$msgs['kelas'] = 'Kelas Diklat belum diisi!';

                if (($value = $this->kelasExists($_POST['kelas'])) && (!Filter::$id)) {
                    if ($value == 3)
                        Filter::$msgs['kelas'] = 'Kelas sudah terdaftar pada Diklat tersebut!';
                }
     ...
     /**
      ...
      /**
       * Content::getRegAwalDiklat_Peserta()-------------------------------------------------------------------------------------------------------------
       * 
       * @return
       */
      public function getRegAwalDiklat_Peserta($jadwalid = 0,$uKota)
      {
            ...
            if (isset($_GET['uKota']))
                $uKota = strtolower(sanitize($_GET['uKota']));
          
            if ($propinsi_kode != '') {
                if ($kota_kode != '') {
                        $q = "SELECT count(*) FROM diklat_peserta AS dp WHERE propinsi_kode = '" . $propinsi_kode . "'  AND dp.reg_awal IS NOT NULL AND dp.reg_ulang IS NULL AND kota_kode = '" . $kota_kode . "'";
                        $sqlwhere = "WHERE dp.propinsi_kode = '" . $propinsi_kode . "' AND dp.kota_kode = '" . $kota_kode . "'";
                      ...
                      if (($searchtext != '') && ($searchfield != '')) {
                        ...
                        $q = "SELECT count(*) FROM diklat_peserta AS dp WHERE propinsi_kode = '" . $propinsi_kode . "' AND dp.reg_awal IS NOT NULL AND dp.reg_ulang IS NULL ";
                        $sqlwhere = "WHERE dp.propinsi_kode = '" . $propinsi_kode . "'";
                ...
                if ($jadwalid > 0) {
                  ...
                  if ($kota_kode != '') {
                        $q = "SELECT count(*) FROM diklat_peserta AS dp WHERE kota_kode = '" . $kota_kode . "' AND dp.reg_awal IS NOT NULL AND dp.reg_ulang IS NULL ";
                        $sqlwhere = "WHERE dp.kota_kode = '" . $kota_kode . "'";
                        ...
                        if ($jadwalid > 0) {
                          ...
                          if (($searchtext != '') && ($searchfield != '')) {
                                ...
                                if ($jadwalid > 0) {
                                    $q .= " WHERE dp.jadwalid = " . $jadwalid . " AND dp.reg_awal IS NOT NULL AND dp.reg_ulang IS NULL ";
                                    $sqlwhere = " WHERE dp.jadwalid = " . $jadwalid;
                                } else {                              
                                    $sqlwhere = "";
                                    $q .= " WHERE dp.reg_awal IS NOT NULL AND dp.reg_ulang IS NULL ";
                                }
            ...
            if($sqlwhere == ""){
              $q = "SELECT count(*) FROM diklat_peserta AS dp WHERE dp.reg_awal IS NOT NULL AND dp.reg_ulang IS NULL ";
            ...
            if (isset($_GET['sortfield'])) {
                ...
                if (in_array($sortfield, array("p.nama_propinsi","k.nama_kota", "totJum"))) {
                    $sort = ($sorttype == 'DESC') ? " DESC" : " ASC";
                    $sqlorder = "LOWER(" . $sortfield . ") " . $sort;
                } else
                    $sqlorder = "LOWER(p.nama_propinsi)";
            } else
                    $sqlorder = "LOWER(p.nama_propinsi)";

            if($uKota == 0){
                        
                $sql = "SELECT (SELECT COUNT(p.nama_propinsi)) total,"
                . "\n p.nama_propinsi," 
                . "\n k.nama_kota," 
                . "\n dj.tahun,"
                . "\n dp.*," 
                    ...
                    if (!$this->Diklat_PesertaPTKExists($personid, $jadwalid))  {
                        ...
                        if ($jenis == 'P') {
                            ...
                            $sql = "SELECT pt.id,pt.gelar_depan1,pt.gelar_depan2,pt.gelar_depan3," 
                                  ...
                                  . "\n pt.ijazah_akhir, pt.fakultas_akhir, pt.jurusan_akhir,pt.tahun_lulus_akhir,"
                                  . "\n pt.tgl_lahir, pt.tmp_lahir, pt.jns_klmn,pt.nama_ibu,"                                    
                                  . "\n pt.agama, pt.propinsi_kode, pt.kota_kode, pt.alamat, pt.telepon1,pt.telepon2,pt.foto,pt.dir_foto," 
                                  . "\n s.nss, s.nama_sekolah, s.status AS status_sekolah, s.nama_pimpinan, s.alamat AS alamat_sekolah," 
                                  . "\n s.propinsi_kode AS propinsi_kode_sekolah, s.kota_kode AS kota_kode_sekolah, s.telepon AS telepon_sekolah,"
                                  . "\n s.fax AS fax_sekolah, s.kodepos AS kodepos_sekolah, s.email, s.website" 
                                  . "\n FROM ptk AS pt" 
...
//webcam
			...
			if($camfoto){
				...
				$data = str_replace("data:image/jpeg;base64,","",$camfoto);
				$dir = date("dmY");  
  			    $targetdir = "../foto/foto_peserta_diklat/".$dir."/";   
                ...
                //disini adalah fungsi untuk membuat nama gambar dengan kreasi sendiri
				$fnames = 'PESERTA_DIKLAT_'.$nip.'.jpg';  
                ...
                if($fname){
                      ...
                      //disini adalah fungsi untuk membuat nama gambar dengan kreasi sendiri
                      $fnames = 'PESERTA_DIKLAT_'.$nip.'.'.$ext_file;  
      ...
      /**
       * Content::regawalDiklat_Peserta()
       * 
       * @return
       */
      public function regawalDiklat_Peserta()
      {
          if (empty(Filter::$msgs)) {

              $sql = "SELECT MAX(no_absen) as maxno_absen" 
                    . "\n FROM diklat_peserta" 
                    . "\n WHERE jadwalid = (SELECT DISTINCT(jadwalid) FROM diklat_peserta WHERE id = " . Filter::$id . ")";

              $prow = self::$db->first($sql);
              $noabsen = $prow->maxno_absen;
              // echo $noabsen;
              $noabsen += 1;
              // echo $noabsen.'aa';
              $data = array('reg_awal' => 'NOW()','no_absen' => $noabsen);
      ...
      public function updateDiklat_Peserta()
      {
            ...
            if ($prow) {
                ...
                if(!empty($foto)){
                  if (!file_exists('../foto/foto_peserta_diklat/'.$dir_foto)) {
                      mkdir('../foto/foto_peserta_diklat/'.$dir_foto, 0777, true);
                  }
                  copy('../foto/foto_ptk/'.$dir_foto.'/'.$foto, '../foto/foto_peserta_diklat/'.$dir_foto.'/'.$foto);
                }
     ...
     /**
      ...
      public function processDiklat_Mata_Tatar()
      {
          ...
          if (empty(Filter::$msgs)) {
              ...
              'kategoriid' => sanitize($_POST['kategori']), 
     ...
     /**
      ...
      public function getDiklat_NilaiPeserta($pesertaid = 0, $jadwalid = 0)
      {
            ...
            $sql = "SELECT dmt.id, dmt.kode, dmt.nama_mata_tatar, dmt.pengajarid, dmt.nama_pengajar,"
                ...
                . "\n FROM diklat_mata_tatar as dmt" 
                . "\n LEFT JOIN diklat_nilai as dn ON dmt.id = dn.mata_tatarid"
                . "\n WHERE dmt.jadwalid = '" . $jadwalid . "' AND dn.pesertaid = '" . $pesertaid . "'"
