	...
	if(domain === '') {
		...
		if(absoluteUrl === 0) {
			...
			//var afterDomain = window.location.pathname;
                        var afterDomain = window.location.href.replace(/.*\/\/[^\/]*/, '');

                        if (afterDomain.charAt( 0 ) === '/' )
                            afterDomain = afterDomain.slice( 1 );
	...
	setTimeout('$("html").removeClass("loadstate")',500);
        ...
        $("#loginForm").validate({
            ...
            messages: {
                        ...
                        username: {
                                required: "Silahkan isi NUPTK, NIP atau Usernamae!",
                                minlength: "Silahkan isi minimal 4 dan maksimal 18 karakter!"
                        },
                        password: {
